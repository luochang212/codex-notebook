...
子 agent 呢？它完全不知道自己被标记成了什么“角色”。没有额外的 system prompt，没有行为约束，没有权限收窄。它的行为完全由 config 决定。
...
说实话，第一次看到这个空文件时我觉得这是个 bug，或者是个还没实现的 TODO。但后来我想明白了：这恰恰是最诚实的设计。Codex 承认自己无法在系统层面强制"explorer 只读不写"这种行为边界，所以它干脆不假装。它把选择权完全交给模型——如果模型认为某个任务适合用 explorer，它就 spawn 一个 explorer；如果模型判断错了，那也不是系统的责任。
...
每个子 agent 的 SessionSource 记录了它被 spawn 时的深度。超过上限就拒绝。没有复杂的资源核算、没有令牌桶、没有优先级队列——就是一个整数比较。对于当前的使用场景（深度通常不超过 3-4 层），这完全够用。
...
但收益也很明显：添加一个新角色只需要写一段 description。不需要新的 handler、新的权限模型、新的运行时分支。整个角色系统的代码量可能还不到 400 行。
