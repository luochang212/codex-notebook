...
但命中条件是**前缀逐字节相同**。如果你在 system prompt 中间插了一段变化的内容（比如 git status），前缀就变了，cache 就 miss。
...
Session 的第一个 Turn（或者 compaction / rollback 之后 baseline 被清空时），系统做一次全量注入：base instructions、完整的 world state、MCP 工具列表、可用 skills。这是"底版"。注入完成后，当前快照成为 baseline。
...
什么时候 baseline 会被清空？两种情况：compaction 重写了历史窗口（ch05），或者 rollback 裁掉了包含初始上下文的 developer bundle。正常对话中，baseline 从第一个 Turn 建立后就一直延续。
