...
flowchart TD
    ...
    B --> C{"消息类型？"}
    C -->|UserInput| D{"有 active turn？"}
    D -->|有| D2["steer：注入当前 turn 的 pending input"]
    D -->|没有| D3["启动新 Turn"]
...
一个 Turn 从用户输入开始，到模型不再需要后续执行时结束。中间可能经历很多"步"（每步 = 一次模型调用 + 可能的工具执行）：

```mermaid
flowchart TD
    A["用户输入"] --> B["Step 1：模型调用"]
    B --> C{"模型需要后续执行？"}
    C -->|"有 tool call 或有 pending input"| D["执行工具，结果回注历史"]
    D --> E["Step 2：再次调用模型"]
    E --> C
    C -->|否| F{"stop hook 判定"}
    F -->|"stop/block"| G["Turn 结束"]
    F -->|"continue"| D
```

Turn 的结束条件：模型不再需要后续执行（没有 tool call，也没有 pending input），并且 stop hook 没有要求继续。没有"最多跑 N 步"的硬限制——源码注释明确说，只要 compaction 工作正常，不需要担心无限循环。
