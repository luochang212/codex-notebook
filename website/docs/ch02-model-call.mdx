...
网络抖了、服务端短暂过载——这类瞬态故障，等一等再试就好了。Codex 用指数退避重试：初始等待 200ms，每次乘以 2，并加一个 0.9–1.1 的随机抖动避免雷群效应。最多 N 次。如果服务端在错误中返回了建议的等待时间（requested delay），重试会优先采用服务端的值，而不是客户端计算的退避。
