...
flowchart TD
    ...
    B -->|"放行"| C["直接执行"]
    B -->|"拒绝"| D["直接拒绝"]
    B -->|"需要审批"| E{"配置了 Guardian reviewer？"}
    E -->|是| F["Guardian 自动审查"]
    E -->|否| E2["交给用户审批"]
    F --> G["调用 guardian 模型"]
    G --> H{"guardian 判定"}
...
注意最后一条：**超时、报错、输出格式错误，一律 deny。** 这叫 fail closed——guardian 出问题时不会静默放行。deny 的直接结果是拒绝操作，不是自动转成人工询问。
...
然后它返回结构化 JSON。其中 `outcome`（allow/deny）是必填字段，`risk_level` 和 `rationale` 可选，还有 `user_authorization` 字段。解析器允许从外围文本中提取 JSON，但如果提取不到有效结果，就当 deny 处理。
...
但对比替代方案（每条命令都弹框让用户确认），guardian 能自动处理大部分“安全但需要确认”的操作。对于高频操作的 coding agent，这个差距决定了宽松 approval policy 是否可用。
