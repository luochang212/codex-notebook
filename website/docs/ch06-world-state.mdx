...
ch04 讲了上下文装配的总流程。这一章只聚焦其中一个子问题：环境信息（子 agent 列表、工作目录、AGENTS.md、权限配置）每步都可能变，怎么告诉模型？
...
比如模型在 step 3 spawn 了一个子 agent，到 step 7 它需要知道当前有哪些子 agent 在跑、它们的状态如何。如果不告诉它，它可能重复 spawn 或基于过时的列表做决策。
...
为什么大多数 section 不做行级 diff？因为 section 通常很短（几十到几百 token），行级 diff 的收益很小，但实现复杂度大增。而且模型不需要知道“子 agent 列表的第 3 项变了”，它只需要看到最新的列表。
